...
public class AgendamentoController {
    ...
    public ResponseEntity<AgendamentoResponseDTO> createAgendamento(@Valid @RequestBody AgendamentoRequestDTO dto) {
    ...
    @Operation(
            ...
            description = "Retorna agendamentos ativos com paginação e filtros opcionais")
    ...
    public ResponseEntity<AgendamentoResponseDTO> updateAgendamento(
            @PathVariable UUID id, @Valid @RequestBody AgendamentoUpdateDTO dto) {
    ...
    public ResponseEntity<AgendamentoResponseDTO> updateStatus(
            @PathVariable UUID id, @Valid @RequestBody AgendamentoStatusDTO dto) {
    ...
    public ResponseEntity<List<AgendamentoResponseDTO>> getAgendamentosByPaciente(@PathVariable UUID pacienteId) {
        List<AgendamentoResponseDTO> responseDTOs = agendamentoService.getAgendamentosByPaciente(pacienteId).stream()
                .map(agendamentoMapper::toResponseDTO)
    ...
    public ResponseEntity<List<AgendamentoResponseDTO>> getAgendamentosByProfissional(
        ...
        List<AgendamentoResponseDTO> responseDTOs =
                agendamentoService.getAgendamentosByProfissional(profissionalId).stream()
                        .map(agendamentoMapper::toResponseDTO)
                        .collect(Collectors.toList());
        return ResponseEntity.ok(responseDTOs);
    }
    ...
    @Operation(
            ...
            description = "Filtra agendamentos entre datas de início e fim")
    @ApiResponse(responseCode = "200", description = "Lista de agendamentos retornada com sucesso")
    public ResponseEntity<List<AgendamentoResponseDTO>> getAgendamentosByPeriodo(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate inicio,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fim) {
        List<AgendamentoResponseDTO> responseDTOs =
                agendamentoService.getAgendamentosByPeriodo(inicio.atStartOfDay(), fim.atTime(LocalTime.MAX)).stream()
                        .map(agendamentoMapper::toResponseDTO)
                        .collect(Collectors.toList());
        return ResponseEntity.ok(responseDTOs);
    }
    ...
    @Operation(
            summary = "Consultar slots disponíveis",
            description = "Retorna horários livres do profissional para uma data")
    ...
    public ResponseEntity<List<LocalDateTime>> getAvailableSlots(
        ...
        List<LocalDateTime> slots =
                agendamentoService.getAvailableSlots(profissionalId, data, duracaoMinutos, capacidadeMaxima);
        return ResponseEntity.ok(slots);
    }

    @PostMapping("/reposicao")
    @Operation(
            summary = "Criar reposição",
            description = "Cria um agendamento de reposição a partir de um agendamento cancelado com direito")
    @ApiResponses({
    ...
    public ResponseEntity<AgendamentoResponseDTO> criarReposicao(@Valid @RequestBody ReposicaoRequestDTO dto) {
    ...
    @Operation(
            summary = "Info de reposições do paciente",
            description = "Retorna informações sobre reposições usadas e disponíveis no mês")
    ...
    @Operation(
            ...
            description = "Cria uma série de agendamentos com base em um padrão de recorrência")
    @ApiResponses({
    ...
    @Operation(
            summary = "Cancelar agendamento da recorrência",
            description = "Cancela um agendamento ou todos os futuros da mesma recorrência")
    @ApiResponses({
    ...
    public ResponseEntity<List<AgendamentoResponseDTO>> cancelarRecorrencia(
            @PathVariable UUID id, @RequestParam(defaultValue = "false") boolean cancelarFuturos) {
